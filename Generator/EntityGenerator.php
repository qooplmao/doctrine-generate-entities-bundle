...
class EntityGenerator
{
     ...
     * @var boolean
     */
    private $fieldVisibility = 'protected';
...
{
<spaces>return $this-><fieldName>->contains($<variableNameSingular>);
}';
...
public function <methodName>(<methodTypeHint>$<variableNameSingular>)
{
<spaces>if (!$this-><hasMethodName>($<variableNameSingular>)) {
<spaces><spaces>$this-><fieldName>->add($<variableNameSingular>);
<spaces><spaces>$<variableNameSingular>->set<entity>($this);
<spaces>}
...
{
<spaces>if ($this-><hasMethodName>($<variableNameSingular>)) {
<spaces><spaces>$this-><fieldName>->removeElement($<variableNameSingular>);
<spaces><spaces>$<variableNameSingular>->set<entity>(null);
<spaces>}
...
';
    ...
    private function generateEntityBody(ClassMetadataInfo $metadata)
    {
        ...
        if ('QoopLmao\Doctrine\GeneratorBundle\Generator\EntityGenerator' === get_class($this)) {
    ...
    private function generateEntityStubMethods(ClassMetadataInfo $metadata)
    {
        ...
        foreach ($metadata->associationMappings as $associationMapping) {
            if ($associationMapping['type'] & ClassMetadataInfo::TO_ONE) {
                $targetEntity = explode('\\', $associationMapping['targetEntity']);
                $targetEntity = $targetEntity[count($targetEntity) - 1];

                $nullable = $this->isAssociationIsNullable($associationMapping) ? 'null' : null;
                if ($code = $this->generateEntityStubMethod($metadata, 'set', $associationMapping['fieldName'], $targetEntity, $nullable)) {
                    $methods[] = $code;
                }
                if ($code = $this->generateEntityStubMethod($metadata, 'get', $associationMapping['fieldName'], $targetEntity)) {
                    $methods[] = $code;
                }
                ...
                if ($code = $this->generateEntityStubMethod($metadata, 'has', $associationMapping['fieldName'], $targetEntity)) {
                    $methods[] = $code;
                }
                if ($code = $this->generateEntityStubMethod($metadata, 'add', $associationMapping['fieldName'], $targetEntity)) {
                    $methods[] = $code;
                }
                if ($code = $this->generateEntityStubMethod($metadata, 'remove', $associationMapping['fieldName'], $targetEntity)) {
                    $methods[] = $code;
                }
                if ($code = $this->generateEntityStubMethod($metadata, 'get', $associationMapping['fieldName'], 'ArrayCollection')) {
                    $methods[] = $code;
                }
            }
        }
    ...
    private function generateEntityStubMethod(ClassMetadataInfo $metadata, $type, $fieldName, $typeHint = null,  $defaultValue = null)
    {
        ...
        if ('\DateTime ' === $variableType) {
            $methodTypeHint = '\DateTime ';
        }
        ...
        $replacements = array(
          ...
          '<variableNameSingular>' => Inflector::singularize($fieldName),
    ...
    private function generateLifecycleCallbackMethod($name, $methodName, $metadata)
    {
        ...
        $method = str_replace(
            array_keys($replacements),
            array_values($replacements),
            $this->lifecycleCallbackMethodTemplate
        );
